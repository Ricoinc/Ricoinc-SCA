...
<ul class="global-views-breadcrumb" itemprop="breadcrumb" >
	{{#each pages}}
		{{#if @last}}
			<li class="global-views-breadcrumb-item-active" style="color:#5f6369;">
				{{text}}
			</li>
		{{else}}
			<li class="global-views-breadcrumb-item" >
				<a href="{{href}}"
					...
					{{#if hasDataHashtag}} data-hashtag="{{data-hashtag}}" {{/if}} style="color:#343741;"
				> {{text}} </a>
			</li>
			<li class="global-views-breadcrumb-divider"><span class="global-views-breadcrumb-divider-icon" style="color:#343741;"></span></li>
		{{/if}}
	{{/each}}
</ul>
