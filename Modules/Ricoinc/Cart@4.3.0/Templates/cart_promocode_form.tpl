		...
		<div class="cart-promocode-form-summary-container-input">
			<div class="{{#if showErrorMessage}}error{{/if}}">
				...
				<input
					...
					style="border-color:#5f6369;outline: none; color:#5f6369"
				>
			</div>
		</div>
		<div class="cart-promocode-form-summary-promocode-container-button">
			<button type="submit" class="cart-promocode-form-summary-button-apply-promocode" {{#if isSaving}}disabled{{/if}} style="color: white; border: none; background-color:#cd163f;">
				{{translate 'Apply'}}
			</button>
		</div>
	</div>
